...
class Rho(BaseLend):
    name = "Rho"
    contract_address = "0x639355f34Ca9935E0004e30bD77b9cE2ADA0E692"
    supported_tokens = ["ETH", "USDC", "USDT"]
    supported_tokens_for_borrow = []
    lend_tokens = [
        Token("rETH", "0x639355f34Ca9935E0004e30bD77b9cE2ADA0E692", 18, "scroll"),
        Token("rUSDC", "0xAE1846110F72f2DaaBC75B7cEEe96558289EDfc5", 18, "scroll"),
        Token("rUSDT", "0x855CEA8626Fa7b42c13e7A688b179bf61e6c1e81", 18, "scroll"),
    ]
    ...
    def get_points(self, sender: BaseAccount):
        token = self.login(sender)["data"]["signIn"]

        data = {"operationName":"UGetUserPoint","query":"query UGetUserPoint($id: String!) {\n  getUserPoints(userId: $id) {\n    id\n    lastUpdated\n    duration\n    points\n    rank\n    totalInviterRewards\n    rankDifference\n    isSuperBoostUser\n    isNFTOwner\n    assets {\n      id\n      assetType\n      borrowAmount\n      supplyAmount\n      currentScore\n      earnedPoints\n      level\n      inviterReward\n      multiplier\n      bonusTimes\n      updatedAt\n      __typename\n    }\n    __typename\n  }\n}","variables":{"id":sender.address}}

        headers = {"authorization": "Bearer " + token}

        resp = req_post("https://rhomarkets-backend.xyz/graphql", json=data, headers=headers, proxies=sender.proxies)
        
        return resp["data"]["getUserPoints"]["points"]


    def login(self, sender: BaseAccount):
        to_sign = f"Sign User:{sender.address} log in Rho Markets! Timestamp: {int(time()*1000)}"

        auth_msg = "0x"+ to_sign.encode().hex()
        encoded_auth_msg = encode_defunct(hexstr=auth_msg)
        w3 = sender.get_w3('scroll')
        signed_auth = w3.eth.account.sign_message(encoded_auth_msg, private_key = sender.private_key).signature.hex()

        data = {"operationName":"SignIn","query":"mutation SignIn($message: String!, $signature: String!) {\n  signIn(message: $message, signature: $signature)\n}",
                "variables":{"message":to_sign,"signature":signed_auth}}

        resp = req_post("https://rhomarkets-backend.xyz/graphql", json=data, proxies=sender.proxies)
    ...
    def create_txn_for_adding_token(self, token: Token, amount: float, sender: BaseAccount):
        ...
        if token.symbol == "ETH":
            ...
            txn["to"] = self.lend_token_from_token[token.symbol].contract_address
            txn["data"] = "0x1249c58b"
        else:
            txn = txn_data_handler.get_txn_data()
            txn["to"] = self.lend_token_from_token[token.symbol].contract_address
            txn["data"] = "0xa0712d68" + encode(["uint256"], [int(amount*10**token.decimals)]).hex()
        return txn
    ...
    def create_txn_for_removing_token(self, token: Token, sender: BaseAccount):
        ...
        if token.symbol == "ETH":
            txn = txn_data_handler.get_txn_data()
            txn["to"] = token.contract_address
            txn["data"] = "0x852a12e3" + encode(["uint256"], [amount]).hex()
        else:
            txn = txn_data_handler.get_txn_data()
            txn["to"] = token.contract_address
            txn["data"] = "0xdb006a75" + encode(["uint256"], [amount]).hex()
